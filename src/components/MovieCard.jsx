function MovieCard({movie, setSeeDetail, setView}){
    return(
        ...
        className="fade-in w-full h-full flex flex-col my-4 bg-bg-3 border border-line rounded-xl rounded-tl-none pt-4 px-2 cursor-pointer hover:bg-ink-4 hover:border-accent-2">
            <img className="w-full object-cover rounded-xl rounded-tl-none mb-5 mx-auto" 
            src={movie.poster_path ? `https://image.tmdb.org/t/p/w500${movie.poster_path}` : 'https://placehold.co/500x750?text=No+Poster'}
            alt={movie.title}
            />
            <div className="px-4 min-h-full flex-1 my-auto">
                <h2 className="font-fraunces font-medium italic text-xl line-clamp-4 ">{movie.title}</h2>
